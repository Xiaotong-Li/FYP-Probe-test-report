 ...
 * note: Bugfix the incorrect output length by Jerzy Dziewierz
 *=============================================================*/
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    /* if there is two arguments, ignore 1st one and check the 2nd one*/
    inputidx=0;
    if (nrhs!=1) 
    {
        if (nrhs!=2)
        {
            mexErrMsgTxt("One input required, if there are two inputs, first is ignored");
        }
        inputidx=1;
    }
    ...
    if ( mxIsChar(prhs[inputidx]) != 1)
      mexErrMsgTxt("Input must be a string.");

     /* get the length of the input string */
    buflen = (mxGetM(prhs[inputidx]) * mxGetN(prhs[inputidx])) + 1;

    /* allocate memory for input and output strings */
    input_buf=mxCalloc(buflen, sizeof(char));
    
    /* copy the name from command line to input_buf */
      status = mxGetString(prhs[inputidx], input_buf, buflen);
    if(status != 0) 
